#!/bin/bash
# ==============================================================================
# Entry script for training Qwen3-8B (dense, thinking-mode) TaskSync agents.
#
# Differences vs train_megatron.sh (which targets Qwen3-Coder-30B-A3B MoE):
#   - default model_path -> Qwen3-8B
...
exp_name=${exp_name:-"${DATE}-qwen3_8b-${loss_mode}-tp${actor_tp}-pp${actor_pp}-cp${actor_cp}-total_epochs${total_epochs}-group_size${env_group_size}-reward_type${reward_type}${dense_epoch_suffix}${suffix_str}"}
