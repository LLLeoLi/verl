...
class StreamingXMLToolCallParser:
    ...
    def reset_streaming_state(self):
        ...
        self.parameters = {}
        self.function_seen_in_call = False
        ...
        while self.last_processed_pos < len(self.streaming_buffer):
            ...
            try:
                ...
                if (
                    ...
                    self._finish_current_tool_call()
                # If a second <function> starts inside the same <tool_call>,
                # the model emitted malformed XML (e.g. an empty placeholder
                # function followed by the real one). Treat the new function
                # as the start of a NEW tool call so that the arguments of
                # both functions are NOT concatenated into invalid JSON like
                # '{}{...}' (which breaks json.loads with
                # 'Extra data: line 1 column 3 (char 2)').
                elif (
                    (
                        element.strip().startswith("<function=")
                        or element.strip().startswith("<function name=")
                        or element.strip().startswith("<function>")
                    )
                    and self.function_seen_in_call
                    and self.current_call_id
                ):
                    self._finish_current_tool_call()
                # If expat cannot match this </tool_call> (an unclosed
                # <function>/<parameter>, or a <tool_call> auto-created by a
                # bare <function> tag), it raises 'mismatched tag' and the
                # closing is silently dropped, leaving the tool call
                # unterminated (missing '}'/'{}'). Close it manually.
                if (
                    element.strip() == self.tool_call_end_token
                    and (self.current_function_open or self.current_param_name)
                ):
                    self._end_element("tool_call")
                else:
    ...
    def _finish_current_tool_call(self):
        """Close any open elements, emit the final tail delta for the current
        tool call, and reset the parser state. Used when a new <tool_call> or
        a second <function> inside the same <tool_call> is detected."""
        if self.current_param_name:
            self._end_element("parameter")
    ...
    def _auto_close_open_parameter_if_needed(self, incoming_tag: str | None = None):
        ...
        # and there is an unclosed function, close function first
        if incoming_tag in ("function", "tool_call") and self.current_function_open:
            self._end_element("function")
    ...
    def _start_element(self, name: str, attrs: dict[str, str]):
        ...
        if name == "tool_call":
            ...
            self.function_seen_in_call = True
            if function_name:
                delta = DeltaMessage(
                    tool_calls=[
                        DeltaToolCall(
...
class Qwen3XMLToolParser(ToolParser):
    ...
    @staticmethod
    def _repair_arguments(arguments: str) -> str:
        """Validate assembled tool-call arguments; if they are not valid JSON
        (e.g. multiple functions inside one <tool_call> produced a
        concatenation like '{}{...}' or '{}{}{...'), extract the last valid
        JSON object so that json.loads() succeeds. Falls back to '{}'."""
        ...
        # Scan '{' positions from right to left; a candidate must consume the
        # whole remaining string (tolerating a missing trailing '}').
        for i in range(len(arguments) - 1, -1, -1):
            if arguments[i] != "{":
                continue
            for suffix in ("", "}"):
                candidate = arguments[i:] + suffix
                try:
    ...
    def extract_tool_calls(
        ...
        if not result.tool_calls:
            ...
            for tool_call in result.tool_calls:
                if tool_call.function and tool_call.function.name:
                    args = self._repair_arguments(tool_call.function.arguments)
                    tool_calls.append(
                        ToolCall(
                            id=tool_call.id,
                            type=tool_call.type,
                    ...
                    self.prev_tool_call_arr[tool_index]["arguments"] = args

                    # Update streamed arguments
                    if args:
                        self.streamed_args_for_tool[tool_index] = args
